...
test("standalone build ships an executable health worker without tsx", async () => {
  ...
  try {
    ...
    const workerFile = path.join(dir, "src/lib/db/healthCheckWorker.cjs");
    assert.ok(fs.existsSync(workerFile), "standalone build must ship the CommonJS health worker");
