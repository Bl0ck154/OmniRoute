...
export interface CredentialSelectionOptions {
  ...
  /** API-key-scoped ordered preference. Only ranks connections that survived all eligibility gates. */
...
export async function getProviderCredentials(
  ...
  try {
    ...
    let availableConnections = connections.filter((c) => {
      ...
      // of the candidate pool before every routing mode (including explicit
      // probes), so fallback never burns a guaranteed-failing call.
      if (
        provider === "codex" &&
        requestedModel &&
