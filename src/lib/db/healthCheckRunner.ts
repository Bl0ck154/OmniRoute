...
export function resolveDbHealthWorker(): { workerFile: string; execArgv: string[] } {
  ...
  const candidates = [
    // Our current driverFactory still relies on dynamic require() for native SQLite
    // drivers. The packaged worker is therefore CommonJS so those requires remain
    // native instead of being rewritten into unsupported ESM dynamic-require shims.
    path.join(moduleDir, "healthCheckWorker.cjs"),
    path.join(entryDir, "src/lib/db/healthCheckWorker.cjs"),
    path.resolve("src/lib/db/healthCheckWorker.cjs"),
